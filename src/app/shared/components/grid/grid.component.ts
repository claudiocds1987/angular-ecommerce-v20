...
export class GridComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  paginatorConfig = computed<PaginationConfig | null>((): PaginationConfig | null => {
    ...
    effect((): void => {
      ...
      setTimeout((): void => {
        if (this.gridConfigSig().hasInfiniteScroll && this.scrollContainer) {
          ...
          if (el && this._shouldScrollAfterAppend && newData.length > previousLength) {
            ...
            const newRowsHeight = newRowsCount * this.ROW_HEIGHT_PIXELS;
            ...
            const newScroll = Math.max(0, el.scrollHeight - el.clientHeight - newRowsHeight);
  ...
  ngOnDestroy(): void {
    if (this.scrollContainer && this.scrollContainer.nativeElement && this._scrollListener) {
      this.scrollContainer.nativeElement.removeEventListener('scroll', this._scrollListener);
    }
  }

  // funcion que devuelve ej: "Mostrando 50 de 200" solo para paginador de grilla infinita
  public getInfinitePaginatorData(page: number, pageSize: number, length: number): string {
    if (length === 0) {
 ...
 */
  ...
  exportToExcel(): void {
    ...
    if (this._matSort) {
      dataReadyForExport = this._getSortedData(dataReadyForExport, this._matSort);
    }
  ...
  private _syncMatSortState(): void {
    ...
    if (isServerSideSort && this._matSort && orderByConfig) {
      ...
      const direction = (orderByConfig.direction || '') as 'asc' | 'desc' | '';

      // Si el estado ya es correcto, no hacemos nada (esto previene redibujos innecesarios).
      if (this._matSort.active === columnName && this._matSort.direction === direction) {
        return;
      }
  ...
  private _setSortingAccessor(): void {
    this.dataSource.sortingDataAccessor = (
      ...
      const column = this.columns().find((c): boolean => c.name === sortHeaderId);
  ...
  private _applySortAndPaginator(): void {
    ...
    const isClientSidePaginator =
      ...
      (typeof paginatorConfig === 'object' && !paginatorConfig.isServerSide);
  ...
  private _truncate(text: string | number | undefined | null, maxLength = 25): string {
  ...
  private _setupScrollListener(): void {
    if (this._scrollListener && this.scrollContainer && this.scrollContainer.nativeElement) {
      this.scrollContainer.nativeElement.removeEventListener('scroll', this._scrollListener);
    ...
    if (this.gridConfigSig().hasInfiniteScroll && this.scrollContainer) {
      ...
      this._scrollListener = this._ngZone.runOutsideAngular((): (() => void) => {
