...
export class SkeletonDirective {
  ...
  @Input() set appSkeleton(isLoading: boolean) {
    if (isLoading) {
      // Si esta cargando se aplica la clase de estilo skeleton-loading de shared/styles/skeleton.scss
    ...
    setTimeout((): void => {
      ...
      if (!this.subscriptWrapperClass) {
        ...
        this.subscriptWrapperClass = this.el.nativeElement.querySelector(
      ...
      if (this.subscriptWrapperClass) {
        if (isLoading) {
          ...
          this.renderer.setStyle(this.subscriptWrapperClass, 'display', 'none');
        } else {
          // Para restaurar el estilo .mat-mdc-form-field-subscript-wrapper definido por material
          this.renderer.removeStyle(this.subscriptWrapperClass, 'display');
        }
      }
    }, 0);
  }
